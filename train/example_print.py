...
##########################################################################################################################################
DnCNN_model = DnCNN()
optim = torch.optim.Adam(DnCNN_model.parameters(), lr=1e-4, weight_decay=1e-4)


"""하나의 프레임만 썼을 때의 불러오는 이미지의 개수: 35=카메라 종류 수"""
# img_dict_list, num_images = make_dir_dict_list(frame_index=12)
# print(num_images) # 35
...
train_dataloader, test_dataloader = prepare_dataloader(dataset, batch_size=50, scale=1., prop=.8)
...
print(train_dataloader.dataset[0].shape)
print("[Train Start]")
for epoch in range(start_epoch, max_epoch):
    ...
    for batch_idx, x in enumerate(train_dataloader):
        ...
        if bt_sz < 50:
            continue
        x = x.reshape([bt_sz*4, 3, pt_sz, pt_sz])
        optim.zero_grad()
        # print(x.shape) #torch.Size([50, 3, 256, 64])
        output, res = DnCNN_model(x)
        output = output.reshape([bt_sz* 4, pt_sz, pt_sz])
        # print(output.shape, res.shape) #torch.Size([50, 1, 256, 64]) torch.Size([50, 3, 256, 64])
        # print(label.unsqueeze(1).shape, ptch1.shape, len(res1))
        
        loss = loss_fn(output) # [200,64,64]
        ...
        optim.step()
    print('[', epoch, 'Done. Total loss in this epoch:', sum(losses)/len(losses) )

snapshot = {
            "MODEL_STATE": DnCNN_model.state_dict(),
            "EPOCHS_RUN": epoch,
            "Loss": sum(losses)/len(losses) ,
            # "Acc": self.acces.avg,
        }
torch.save(snapshot, '/media/data2/jiwon/snapshot_videonosiemodel_full.pt') # 저장 방식에 대한 고찰 요구됨 